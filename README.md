...
*   **Navigation** [![Nalu.Maui.Navigation NuGet Package](https://img.shields.io/nuget/v/Nalu.Maui.Navigation.svg)](https://www.nuget.org/packages/Nalu.Maui.Navigation/) [![Nalu.Maui NuGet Package Downloads](https://img.shields.io/nuget/dt/Nalu.Maui.Navigation)](https://www.nuget.org/packages/Nalu.Maui.Navigation/)
    *   Offers a fluent, type-safe MVVM navigation service built on `Shell`, supporting relative/absolute navigation, guards, and parameter passing. Includes a leak detector and opt-in [navigation state restoration](conceptual_docs/navigation-restore.md) (land exactly where you were after an app restart — currently verified with the Scaffold host). Also provides a customizable tab bar feature (iOS/Android/MacCatalyst) that works with both standard Shell and NaluShell.
*   **Scaffold** 🧪 **Preview — published to GitHub Packages only, not yet on NuGet.org**
    *   A complete, Nalu-drawn application shell replacing MAUI `Shell` on **iOS and Android**: tab bar (with overflow), nav bar, edge drawers, popups & bottom sheets, page transitions with shared elements, scroll-driven chrome and system-bar handling — every piece is a MAUI view you can restyle or replace. It hosts Nalu navigation unchanged, so tab taps, back gestures and flyout selections all route through the engine and guards/lifecycle always run.
    *   📦 **Availability:** pre-release packages are published to [GitHub Packages](https://github.com/nalu-development/nalu/packages) while the API settles — you'll need to add that source (GitHub authentication required) to restore them. Nothing is on NuGet.org yet.
    *   `Nalu.Maui.Navigation` keeps working with MAUI `Shell` exactly as before: the Scaffold is an **additional host**, not a breaking change.
    *   📖 Docs: [Overview](conceptual_docs/scaffold.md) · [Structure & Tab Bar](conceptual_docs/scaffold-structure.md) · [Nav Bar](conceptual_docs/scaffold-navbar.md) · [Flyouts](conceptual_docs/scaffold-flyout.md) · [Popups & Sheets](conceptual_docs/scaffold-overlays.md) · [Transitions](conceptual_docs/scaffold-transitions.md) · [Migrating from NaluShell](conceptual_docs/scaffold-migration.md) — these link to the repository, since the documentation website only publishes on releases.
