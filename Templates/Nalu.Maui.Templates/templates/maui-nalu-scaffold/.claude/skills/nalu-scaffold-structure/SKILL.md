...
- On a WINDOWED iPad (iPadOS 26) the system draws its window controls over the window's top-leading corner, covering whatever is there. Put a `<nalu:ScaffoldSystemWindowControlsSpacer />` first in a custom bar's leading row (`Orientation="Vertical"` first in custom start-drawer content) — it is zero-sized on Android, on iPhone and on a full-screen iPad, so it costs nothing elsewhere. The default nav bar and `ScaffoldFlyoutMenuView` already carry one.
