...
- `NavBarAppearance` never writes into view properties; `Foreground` is only the primitives' color FALLBACK — an explicit (or styled) `IconColor`/`TextColor` wins and PINS that color; the template styles neither, so page-level `Foreground` recolors title and buttons together. Do not add `TextColor`/`IconColor` setters to `ScaffoldNavBarTitle`/`ScaffoldNavBarButtonBase` styles unless the color must never follow the page. Defaults: `Background` #F7FFFFFF, `Opacity` 1, `OffsetY` 0. An appearance inside a shared `Style` is one object for all pages — constants only.
