...
public class VisualRegressionTester
{
    ...
    {
        var imageFileName = $"{name}{actualImage.Format.GetFileExtension()}";

        var snapshotsEnvironmentDirectory = GetEnvironmentDirectory(_snapshotsDirectory, environmentName);
        var baselineImagePath = Path.Combine(snapshotsEnvironmentDirectory, imageFileName);

        var diffEnvironmentDirectory = GetEnvironmentDirectory(_snapshotsDiffDirectory, environmentName);
        var diffDirectoryImagePath = Path.Combine(diffEnvironmentDirectory, imageFileName);
        var diffDirectoryDiffImagePath = Path.Combine(diffEnvironmentDirectory, name + "-diff.png");
        var ciGetImagesText = testContext != null ? "See test attachment or download the build artifacts to get the new snapshot file." : "Download the build artifacts to get the new snapshot files.";
        ...
        if (!File.Exists(baselineImagePath))
        {
            ...
            var copyCommand = RuntimeInformation.IsOSPlatform(OSPlatform.Windows) ? "copy" : "cp";
        ...
        var imageDifference = _visualComparer.Compare(baselineImage, actualImage);
        if (imageDifference != null)
        {
            Directory.CreateDirectory(diffEnvironmentDirectory);
            actualImage.Save(diffEnvironmentDirectory, name);

            var diffImage = _visualDiffGenerator.GenerateDiff(baselineImage, actualImage);
        ...
        else
        {
            ...
            DeleteFileIfExists(diffDirectoryImagePath);
            DeleteFileIfExists(diffDirectoryDiffImagePath);
        }
    }
