...
public static partial class SoftKeyboardManager
{
        ...
        if (_adjustMode == SoftKeyboardAdjustMode.Resize)
        {
            ...
            UIView.Animate(
                ...
                () =>
                {
                    ...
                    var parentScrollView = FindParentScroll(_textView.FindPlatformResponder<UIScrollView>());
    ...
    private static UIScrollView? FindParentScroll(UIScrollView? view)
    {
    ...
    private static bool IsHorizontalCollectionView(UIView collectionView)
    {
        if (collectionView is UICollectionView cv)
        {
            if (cv.CollectionViewLayout is UICollectionViewFlowLayout flowLayout)
            {
                return flowLayout.ScrollDirection == UICollectionViewScrollDirection.Horizontal;
            }

            if (OperatingSystem.IsIOSVersionAtLeast(13) && cv.CollectionViewLayout is UICollectionViewCompositionalLayout compositionalLayout)
            {
#pragma warning disable CA1416
                return compositionalLayout.Configuration.ScrollDirection == UICollectionViewScrollDirection.Horizontal;
