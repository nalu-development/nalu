...
- **Port model**: this repo pins PER-PLATFORM ports via `AddMauiDevFlowAgent(o => o.Port = …)`
  in both the TestApp and the DailyHelper — Android **9223** (host side reached through
  `adb forward tcp:9223 tcp:9223`), iOS simulator **9224**, Mac Catalyst **9225**. The
  simulator and Catalyst bind the Mac's loopback directly, so per-platform ports let the
  emulator and simulator apps run at the same time (and `DEVFLOW_PORT` selects the target).
  When a port lingers in TIME_WAIT after a relaunch the broker assigns **port+1000**
  (10223/10224/10225); `NaluApp` probes all six. One app instance per platform at a time —
  two iOS apps still contend for 9224 (the loser lands on 10224).
